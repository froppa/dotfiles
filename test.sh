...
[[ "$(grep -oE '^keybind = ctrl\+alt\+super\+[^=]+' home/dot_config/ghostty/config.ghostty)" == "$(grep -E '^keybind = ' home/dot_config/ghostty/herdr.ghostty | sed -E 's/=(unbind|ignore)$//')" ]] || fail "herdr mode must unbind or ignore exactly the Hyper chords config.ghostty sends to tmux"
