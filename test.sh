...
! grep -Fq 'keybind = super+k=' home/dot_config/ghostty/config.ghostty || fail "Command-K must fall through to Ghostty's clear_screen so it clears the scrollback too"
grep -Fq 'confirm-close-surface = true' home/dot_config/ghostty/config.ghostty || fail "closing a surface with a process still running must confirm now that tmux does not"
grep -Fq 'keybind = global:ctrl+alt+super+t=toggle_quick_terminal' home/dot_config/ghostty/config.ghostty || fail "the scratch terminal chord must stay global to replace tmux display-popup"
